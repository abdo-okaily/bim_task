...
class UserService
{
    ...
    {
        $user = $this->repository->all()->where("phone", $request->phone)
        ->where('type','customer')
        ->first();
        ...
        if(!empty($user)) {
            ...
            $code = App::environment('production') ? rand(1000, 9999) : 1234;
            ...
            return [
                "isSuccess" => true,
                "statusCode" => 200,
                "data" => [$request->phone, $code], //@todo Change-Code 
    ...
    {
        $user = $this->repository->all()->where("phone", $request->phone)->where("type", UserTypes::CUSTOMER)->where("verification_code", $request->code)->where('is_banned',0)->first();

        if(empty($user) || now()->timestamp > (int) $user->verification_code_expiration_date) {
            return [
                "isSuccess" => false,
                "statusCode" => 400,
                "data" => null,
        ...
        return [
            ...
            "message" => trans("customer.login.success-login")
        ];
    }
    ...
    private function _updateUserVerificationFieldsOnGenerate(User $user, int $code) : void
    {
        $this->repository->update([
            ...
            "verification_code_expiration_date" => Carbon::now()->addMinutes(30)->timestamp
        ], $user);
    }
    ...
    private function _saveForgetPasswordCode($phone) : string
    {
        if(App::environment('production')) {
        ...
        $message=__('translation.your_reset_password_code_is').': '.$code;
        return $message;
    }

    public function _saveForgetPasswordCodeGetMessage(string $phone, string $userType)
    {
        $user=$this->getUserUsingPhone($phone, $userType);
        $message=$this->_saveForgetPasswordCode($phone);
