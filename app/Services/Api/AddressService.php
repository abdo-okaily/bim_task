...
class AddressService
{
    ...
    {
        $addresses =  $this->repository->all()->where('user_id',auth('api')->user()->id)->get();
        return  [
            'success'=>true,
            'status'=>200 ,
    ...
    public function getCurrentAddressUsingID(int $id) : array
    {
        ...
        if($address == null || $address->user_id != auth('api')->user()->id)
            return  [
                'success'=>false,
                'status'=>500 ,
                'data'=>[] ,
    ...
    public function createAddress(array $request) :array
    {
        ...
        if($request['is_international']){
            ...
            $request['international_city'] = null;
        }
        ...
        if($address->is_default){
            ...
            $this->deactiveRestAddresses($address->id);
        }
    ...
    public function updateAddress(Request $request) : array
    {
        ...
        $request =$request->only(['first_name', 'last_name','description' ,'type','is_default','phone','country_id','area_id','city_id','international_city'
        ,'is_international']);
        
        $address = $this->repository->update($request,$address);

        return  ['success'=>true, 'status'=>200 ,'data'=>new AddressResource($address),'message'=> __('address.api.updated_successfuly')];
    }
