...
class VendorWalletService
{
     ...
     */
    public function getAllVendorWalletsWithPagination(int $perPage = 10, string $orderBy = "DESC", Request $request): LengthAwarePaginator
    {
            ...
            ->when(
                ...
                fn($q) => $q->where(
                    fn($subQ) => $subQ->whereHas(
                        'vendor.owner',
                        fn($ownerQ) => $ownerQ->where('name', 'LIKE', "%$search%")
                    )
                    ->orWhereHas(
                        'vendor',
                        fn($vendorQ) => $vendorQ->where(
                            fn ($_q) => $_q->where('name->ar', 'LIKE', "%$search%")->orWhere('name->en', 'LIKE', "%$search%")
                        )
                    )
                )
            )
    ...
    {
        $subtractedBalanceToHalala = $request->amount * 100;
