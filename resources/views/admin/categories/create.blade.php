                ...
                <div class="card-body">
                    <form action="{{ route('admin.categories.store') }}" method="post" class="needs-validation row"  autocomplete="on" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label" for="name_ar">@lang('admin.categories.name_ar') <span class="text-danger">*</span></label>
                            <input type="text" name="name_ar" class="form-control"
                                   value="{{ old("name_ar") }}"
                                   id="name_ar"
                                   placeholder="{{ trans('admin.categories.name_ar') }}">
                            @error('name_ar')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label" for="name_en">@lang('admin.categories.name_en') <span class="text-danger">*</span></label>
                            <input type="text" name="name_en" class="form-control"
                                   value="{{ old("name_en") }}"
                                   id="name_en"
                                   placeholder="{{ trans('admin.categories.name_en') }}">
                            @error('name_en')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
